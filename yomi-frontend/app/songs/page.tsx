...
export default function SongsPage() {
  ...
  // States עבור ה-Flashcards (העתקנו מה-Reader)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (user) {
        formData.append('userId', user.id); // שולחים את המזהה לבאקנד
      }

      const response = await processSongWithGemini(formData);
      
      // השרת עכשיו מחזיר לנו את הקישור לאודיו מהסטורג' ואת המילים!
  ...
  // 🌟 הפונקציות של ה-Flashcards (כמו ב-Reader)
  ...
  const handleSaveFlashcard = async () => {
    ...
    try {
      ...
      await saveFlashcard({
        ...
        storyId: 'song-flashcard', // אפשר לשנות למזהה השיר האמיתי אם תרצה
        kanji: selectedWord.word,
        reading: selectedWord.reading,
        meaning_hebrew: selectedWord.base_form, // כאן נוכל בהמשך להוסיף תרגום אמיתי מג'מיני!
        type: 'song-word'
      });
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 w-full relative">
        ...
        <div className="bg-white dark:bg-[#1E293B] rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col gap-6">
          <div>
            ...
            <input 
              ...
              className="block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 dark:file:bg-blue-900/20 dark:file:text-blue-400 hover:file:bg-blue-100 transition-all cursor-pointer"
            />
          </div>
          <div>
          ...
          >
            {isLoading ? (
              <><Loader2 className="animate-spin" size={20} /> מנתח ומעלה למסד הנתונים...</>
            ) : (
              <><UploadCloud size={20} /> צור שיעור אינטראקטיבי</>
            )}
          </button>
        </div>
      )}
      ...
      {syncData && (
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white dark:bg-[#1E293B] rounded-2xl p-6 md:p-10 shadow-sm border border-gray-100 dark:border-gray-800 leading-loose text-center min-h-[50vh] pb-32">
      ...
      {/* 🌟 הפופ-אפ של ה-Flashcard מועתק ישירות מה-Reader! */}
      <AnimatePresence>
        {selectedWord && (
          <motion.div
          ...
          >
            ...
            <div className="text-center mb-7">
              ...
              <div className="text-2xl text-blue-600 dark:text-blue-400 font-semibold">{selectedWord.base_form}</div>
            </div>
            ...
            <div className="flex justify-center gap-4 border-t border-gray-100 dark:border-gray-800 pt-5">
              ...
              <button 
                ...
                disabled={isSaving}
                className="flex-1 flex items-center justify-center gap-2 bg-blue-600 dark:bg-gradient-to-r dark:from-blue-600 dark:to-indigo-600 text-white px-5 py-3 rounded-xl font-bold hover:bg-blue-700 dark:hover:from-blue-700 dark:hover:to-indigo-700 transition-colors disabled:opacity-50 shadow-md"
              >
